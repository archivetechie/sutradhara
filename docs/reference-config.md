...
|---|---|---|
| `SUTRADHARA_DB_URL` | `sqlite:///./sutradhara.db` | SQLAlchemy URL of the catalog database. The default is a SQLite file in the current working directory, so long-lived deployments should always set this. Alembic resolves the same variable (`alembic/env.py`). SQLite writer engines use WAL, foreign-key enforcement, a 30-second busy timeout, and `synchronous=FULL` because retention, enrollment, and idempotency records are not rebuildable from storage backends. (`catalog/session.py`) |
