...
| `api/` | The FastAPI operator console API (`/api/...`), identity/capability parsing, and the read models the browser console consumes. |
| `grpc/` | The mTLS gRPC server: streaming intake (`IntakeService`) and the device relay (`DeviceService`), plus enrollment admin. |
...
<!-- code-anchor: src/sutradhara/catalog/models.py src/sutradhara/catalog/types.py @ 5c44b85 -->
## Data model
...
`purged`) and `AssetDerivation` (provenance edges between items, e.g.
transcodes).
...
2. **Register** (`intake.py`, `sutra intake register|watch`). Three
   ...
   invalid never-accepted bags are quarantined. `sutra intake watch` runs
   this as a polling registrar over the landing filesystem.
...
<!-- code-anchor: src/sutradhara/api src/sutradhara/grpc proto @ 5c44b85 -->
## Operator surface
...
whole thing for one-click laptop setup.

<!-- code-anchor: alembic @ 5c44b85 -->
## Migrations

Schema history is alembic, in `alembic/`: 28 migrations in a single
linear chain from `ea7254a77d7a` (the initial
logical-asset/backend/copy tables) to head `c9a0d1e2f3b4` (copy-grain
durability schema). The chain tracks the system's growth: job table,
pools, staging transforms, leases, the reconciler spine, arrangements and
submissions, virtual arrangements, retention, gRPC intake and relay
state, the hdcache tier, and restore admission. `alembic/env.py` resolves
the database from `SUTRADHARA_DB_URL` exactly like the runtime. `sutra db
init` (create-all) exists for development; production schema changes go
