...
> Status: **implemented** (2026-06-30; brainstorm 2026-06-30, Claude + Ada;
...
`ada-macbook-01`), **not** a person. The cert's SHA-256 fingerprint is the durable
device handle.
...
sutra-agent enroll \
  ...
  --device-id ada-macbook-01 \   # CN of the device cert
  --operator ada \               # who this device's intakes are stamped as
...
**Revocation:** on a lost machine, run `sutra serve-grpc --revoke-device ada-macbook-01`
...
// Streaming mode (new — server required)
{
  ...
  "device_id":   "ada-macbook-01",   // informational; the device cert CN is authoritative
  "source_kind": "card",
...
- **2026-06-30 — brainstorm (Claude + Ada).** Settled: single-pass streaming over
