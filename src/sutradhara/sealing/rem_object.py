...
REM_OBJECT_CHUNK_SIZE = 262144
REM_OBJECT_TIMESTAMP = "2026-01-01T00:00:00Z"

_DIGEST_SIZE = 32
_REM_REPRESENTATIONS = {
    "plaintext": Representation.REM_OBJECT_V1,
    "encrypted": Representation.REM_ENCRYPT_V1,
}
_ID_LABELS = {
    "object_id": b"sutradhara:rem-object:object-id:v1\0",
    "caller_object_id": b"sutradhara:rem-object:caller-object-id:v1\0",
    "manifest_file_id": b"sutradhara:rem-object:manifest-file-id:v1\0",
}


@dataclass(frozen=True)
class RemObjectInspection:
    """Keyless Remanence REM-OBJECT inspection mapped into Sutradhara vocabulary."""

    representation: Representation
    format_version: int | None
    recipient_epochs: tuple[RemObjectRecipientEpoch, ...]
    report: dict[str, Any]


@dataclass(frozen=True)
class RemObjectRecipientEpoch:
...
class RemObjectCliSealer:
    """Seal local files by shelling out to Remanence's REM-OBJECT CLI."""
        ...
        with tempfile.TemporaryDirectory(
            ...
            if representation is Representation.REM_OBJECT_V1:
                ...
                if key_epoch is None:
                    raise ValueError("rem-encrypt-v1 sealing requires key_epoch")
            ...
            if file_digest != plaintext_digest:
                raise RuntimeError(
                    "REM-OBJECT build file_sha256 differs from source digest: "
            ...
            if report_stored_digest != local_stored_digest:
                raise RuntimeError(
                    "REM-OBJECT build stored_digest differs from sealed bytes: "
                    f"{report_stored_digest.hex()} != {local_stored_digest.hex()}"
                )
            report_recipients = (
                recipient_registry_ids(report, failure_label="REM-OBJECT build")
                if representation is Representation.REM_ENCRYPT_V1
                else ()
            )
            if (
                representation is Representation.REM_ENCRYPT_V1
                and report_recipients != recipient_ids
            ):
                raise RuntimeError(
                    "REM-OBJECT build recipient epochs differ from registry selection: "
...
class RemObjectCliOpener:
    """Open local Remanence REM-OBJECT objects back to plaintext via the CLI."""
    ...
    def open(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory(
            prefix="sutradhara-rem-object-open-",
            dir=_scratch_dir(work_dir, self._work_dir),
        ) as temp_dir_raw:
            temp_dir = Path(temp_dir_raw)
            os.chmod(temp_dir, 0o700)
            if representation is Representation.REM_OBJECT_V1:
                _extract_rem_object(source, temp_dir, representation=representation)
            elif representation is Representation.REM_ENCRYPT_V1:
                if recipient_epochs is None:
                    raise ValueError("rem-encrypt-v1 opening requires recipient_epochs")
...
def inspect_rem_object(path: Path | str) -> RemObjectInspection:
    """Inspect a REM-OBJECT object and recover its Sutradhara representation.

    Encrypted REM-OBJECT headers expose recipient identities without key material.
    Plaintext REM-OBJECT inspection needs the contract chunk size because the stored
    object is a fixed-block tar stream.
    """
    source = Path(path)
    args = ["archive", "inspect", "--object", str(source)]
    if not _looks_encrypted(source):
        args.extend(["--chunk-size", str(REM_OBJECT_CHUNK_SIZE)])
    ...
    if not isinstance(rem_representation, str):
        raise RuntimeError(f"REM-OBJECT inspect did not report representation: {report!r}")
    try:
        representation = _REM_REPRESENTATIONS[rem_representation]
    except KeyError as exc:
        raise RuntimeError(f"unknown REM-OBJECT representation {rem_representation!r}") from exc
    if representation is Representation.REM_ENCRYPT_V1:
        format_version = report.get("format_version")
        if format_version != 2:
            raise RuntimeError(
                f"encrypted REM-OBJECT inspect reported unexpected format_version: {report!r}"
            )
        labels = recipient_registry_ids(report, failure_label="REM-OBJECT inspect")
        raw_recipients = report["recipient_epochs"]
        recipients = tuple(
            RemObjectRecipientEpoch(epoch_id=str(value["epoch_id"]), label=label)
            for value, label in zip(raw_recipients, labels, strict=True)
        )
        return RemObjectInspection(
            representation=representation,
    ...
    *,
    ...
    if representation is Representation.REM_ENCRYPT_V1:
        if len(recipient_epochs) != 2 or len(recipient_files) != 2:
            raise ValueError("encrypted REM-OBJECT build requires hot and recovery recipients")
    elif recipient_epochs or recipient_files:
        raise ValueError("recipients are only valid for encrypted REM-OBJECT builds")
    result = run_rem_archive_build(
        ...
        timestamp=REM_OBJECT_TIMESTAMP,
    ...
    *,
    ...
    if representation is Representation.REM_OBJECT_V1:
        ...
        if private_key is None:
            raise ValueError("encrypted REM-OBJECT extract requires private_key")
        args.extend(["--private-key", str(private_key)])
    else:
        raise ValueError(f"unsupported REM-OBJECT representation: {representation}")
...
def _assert_report_representation(
    ...
    if not isinstance(rem_value, str):
        raise RuntimeError(f"REM-OBJECT build reported unexpected representation: {report!r}")
    try:
        actual = _REM_REPRESENTATIONS[rem_value]
    except KeyError as exc:
        raise RuntimeError(
            f"REM-OBJECT build reported unexpected representation: {report!r}"
        ) from exc
    if actual is not expected:
        raise RuntimeError(
            f"REM-OBJECT build representation drift: {actual.value} != {expected.value}"
        )
    chunk_size = report.get("chunk_size")
    if chunk_size != REM_OBJECT_CHUNK_SIZE:
        raise RuntimeError(
            f"REM-OBJECT build chunk_size drift: {chunk_size!r} != {REM_OBJECT_CHUNK_SIZE}"
        )
...
def _single_member_digest_from_build_report(report: dict[str, Any], basename: str) -> bytes:
    ...
    if not isinstance(files, list):
        raise RuntimeError(f"REM-OBJECT build report has no file list: {report!r}")
    matches = [
        row
        for row in files
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"REM-OBJECT build expected one regular member {basename!r}, got {len(matches)}"
        )
    digest = matches[0].get("file_sha256")
    if not isinstance(digest, str):
        raise RuntimeError(f"REM-OBJECT build file row missing file_sha256: {matches[0]!r}")
...
def _single_restored_member(dest: Path) -> Path:
    ...
    if len(files) != 1:
        raise RuntimeError(
            f"REM-OBJECT extract expected one regular file, got {len(files)} under {dest}"
        )
    return files[0]
    ...
    for line in result.stdout.splitlines():
        ...
        if line.startswith("{") and line.endswith("}"):
            ...
            if isinstance(data, dict):
                return data
    raise RuntimeError(
        "Remanence REM-OBJECT command emitted no JSON object: "
...
def _digest_from_report(report: dict[str, Any], key: str) -> bytes:
    ...
    if not isinstance(value, str):
        raise RuntimeError(f"REM-OBJECT report missing {key!r}: {report!r}")
...
def _digest_from_hex(value: str, *, field: str) -> bytes:
    try:
        ...
        raise RuntimeError(f"REM-OBJECT report {field!r} is not hex: {value!r}") from exc
    if len(digest) != _DIGEST_SIZE:
        raise RuntimeError(
            f"REM-OBJECT report {field!r} must be {_DIGEST_SIZE} bytes, got {len(digest)}"
        )
    return digest
